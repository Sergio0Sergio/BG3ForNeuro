...
- **Storage — `ScriptExtender/Config.json` (via `Ext.IO.LoadFile`, NOT `Ext.Mod.GetConfig`).** The flag is read once at startup; the failure counter lives **in memory** (after a restart the fallback resets and the honest path is back; this is a deliberate trade-off: resilience per session, not forever). Currently there is NO config mechanism in the mod — `use_osi_spell` came per-action from the neuro. Implemented (v0.8.25): `readModConfig()` reads the file with `Ext.IO.LoadFile("Mods/" .. MOD_NAME .. "/ScriptExtender/Config.json", "data")` + `Ext.Json.Parse`; `Ext.Mod.GetConfig` does not exist on this build (verified), so no separate config API was introduced. See spec §6.4a.
