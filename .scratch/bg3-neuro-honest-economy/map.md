...
- [03 — force_legacy flag](issues/03-force-legacy-config.md): HYBRID — the honest path is the default; a failure → auto-pickup of legacy for that call, after N=3 consecutive failures — persistent legacy until restart. One kill-switch for all paths (attacks/casts/bonuses); the per-action `use_osi_spell` stays on top. Storage: Config.json via `Ext.Mod.GetConfig` (introduced for the first time; counter in memory, reset on restart). Documentation in BG3_Neuro_Spec.md.
...
- All 5 map tickets (01-06) are resolved and **executed** on v0.8.25 (on the owner's command):
