...
{
  "alias": "...",
  "perception": "visible",          // всегда "visible"; сущности без него в состоянии нет
  "perception_reason": "cansee"     // опционально, диагностика: какой сигнал дал verdict
}
```

- Перечисление `perception` — **одно значение `visible`** (поле остаётся для явности/тестов), либо вовсе опускается; контракт: **эмисия = видимость**. Значение `known`/`unknown` в v1 не существует.
...
- `perception_reason` — диагностический (значения `cansee` / `los` / `frustum` / `fog` / `acting` и т.п.), не является частью контракта для App.
...
- **Невидимость в бою не скрывает позицию:** BG3 отрисовывает невидимого участника мерцающим полупрозрачным силуэтом на его истинном месте (можно накрыть AoE; атака снимает невидимость). Такие участники эмитятся как все — `perception="visible"`, позиция истинная. Статус невидимости НЕ понижает эмиссию в бою.
...
## 9. Влияние на роутер (тикет 04)

- Роутер гейтит действия по `feasible` (A), НЕ по `perception`. Отсутствие сущности в состоянии = действия по ней токсичны (её нет для Neuro).
...
- Гейты: `issues/04-action-gating.md`; приёмка: `issues/05-acceptance-and-bench.md`.