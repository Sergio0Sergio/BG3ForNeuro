...
- Every resource-spending action (`attack_entity`, `cast_spell`, `bonus_action`) writes `writeResourceSnapshot(id, actor, "before"/"after")` — the full `SNAPSHOT_RESOURCES` set (AP/BA/Movement/Reaction/SpellSlots/Cooldowns). The AP/BA delta is the honest-economy acceptance criterion on the test bench.
